...
description: "Верифицируемый цикл разработки для сложных задач: freeze spec → build → evidence → verify (fresh session) → fix. Создаёт .agent/tasks/ с артефактами, ставит субагентов (spec-freezer, builder, verifier, fixer). Используй когда пользователь говорит 'proof loop', 'верифицируй задачу', 'запусти proof loop', 'task with verification', 'сложная задача с проверкой', 'freeze spec and verify'. НЕ используй для: простых задач без верификации (прямой код), SDD-спеков без proof artifacts (sdd-spec-init), code review без frozen spec (senior-review)."
license: Apache-2.0
