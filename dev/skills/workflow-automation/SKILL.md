...
description: "Workflow автоматизация с кодом: n8n, Temporal, Inngest. Используй когда пользователь говорит 'workflow automation', 'durable execution', 'orchestrator-worker', 'Temporal', 'Inngest', 'автоматизация процессов', 'production workflow', 'reliable automation'. Паттерны: sequential, parallel, orchestrator-worker. НЕ используй для: no-code (zapier-make-patterns), n8n MCP (n8n-mcp-tools-expert), Telegram-ботов (telegram-bot-builder), мульти-агентов (multi-agent-patterns)."
