---
description: "Анализ разрыва между требованиями и существующим кодом. Используй когда пользователь говорит 'gap analysis', 'что не реализовано', 'SDD gap', 'implementation gap', 'чего не хватает'. НЕ используй для: валидации дизайна (sdd-validate-design), валидации имплементации (sdd-validate-impl), статуса (sdd-spec-status)."
