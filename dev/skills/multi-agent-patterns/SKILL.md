...
description: "Паттерны мульти-агентных систем: оркестратор, peer-to-peer, иерархия. Используй когда пользователь говорит 'мульти-агент', 'multi-agent', 'оркестратор агентов', 'agent swarm', 'agent hierarchy', 'координация агентов', 'agent communication'. НЕ используй для: создания субагентов Lance (l-k-subagent), Agent SDK проекта (l-d-agent-sdk), AI-продуктов (ai-product), MCP-серверов (mcp-builder)."
