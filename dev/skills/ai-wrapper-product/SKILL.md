...
description: "Продукты-обёртки над AI API (OpenAI, Anthropic). Используй когда пользователь говорит 'AI wrapper', 'GPT product', 'обёртка над API', 'AI SaaS', 'AI tool', 'ChatGPT но для X', 'AI-продукт на API'. Prompt engineering для продуктов, управление костами, rate limiting, defensibility. НЕ используй для: AI-продуктов вообще (ai-product), micro-SaaS (micro-saas-launcher), Agent SDK (l-d-agent-sdk), мульти-агентов (multi-agent-patterns)."
